...
const { getUnusedFiles } = require('./getUnusedFiles');
...
const promptForFilePath = () => {
    return inquirer.prompt(getFilePath).then(answers => {
...
const isValidAnswer = value => {
    ...
    const isValidAnswer = ['y', 'yes', 'n', 'no', 'r', 'rest', 's', 'skip'].includes(
        value.toLowerCase()
    );
...
const promptForDeleteFile = idx => {
    return inquirer.prompt(questions[idx]).then(answers => {
...
const promptForDeleteRestConfirmation = lastIdx => {
    return inquirer.prompt(areYouSureDelete).then(answers => {
...
const promptForSkipRestConfirmation = lastIdx => {
    return inquirer.prompt(areYouSureSkip).then(answers => {
